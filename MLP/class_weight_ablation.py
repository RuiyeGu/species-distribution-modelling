...
def run():
    # Full labelled data = the two canonical plot-level split files combined.
    # This script does its OWN plot-level resampling (10 seeds), so it needs
    # all 641 plots, not the protocol's fixed dev/test division.
    df = pd.concat([pd.read_csv("train_plotlevel.csv"),
                    pd.read_csv("test_plotlevel.csv")], ignore_index=True)
    rows = []
