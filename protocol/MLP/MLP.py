...
# protocol.py lives one level up; add it to the path so this script runs as
# `python protocol/MLP/MLP.py` from the repo root without PYTHONPATH.
...
class MultiMLPTransformer(BaseEstimator, TransformerMixin):
    """Impute + StandardScale environmental predictors, then append one-hot Species.

    Every step is fit on the training fold only. Scaling of the environmental
    block is what makes this suitable for an MLP; the binary Species dummies are
    concatenated unscaled.
    """
